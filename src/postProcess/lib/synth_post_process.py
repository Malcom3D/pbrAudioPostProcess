...
class SynthPostProcess:
    ...
    def _init_filters(self):
        ...
        self.gate_attack = int(self.synthpostprocess.noise_gate_threshold_db * self.sample_rate / 1000)
        self.gate_release = int(50 * self.sample_rate / 1000)  # 50ms release
        
        # Smoothing filter (low-pass)
        smoothing_cutoff = 1000.0 / (self.synthpostprocess.smoothing_window_ms * 2.0)
    ...
    def process_object(self, obj_name: str, obj_idx: int) -> Dict[str, np.ndarray]:
        ...
        if self.synthpostprocess.blend_enabled and len(processed_tracks) > 1:
    ...
    def _process_track(self, 
        ...
        if self.synthpostprocess.dynamic_denoise_enabled:
        ...
        if self.synthpostprocess.blend_enabled:
        ...
        if self.synthpostprocess.normalize_output:
    ...
    def _apply_noise_gate(self, signal_data: np.ndarray, force: np.ndarray) -> np.ndarray:
        ...
        threshold_linear = 10 ** (self.synthpostprocess.noise_gate_threshold_db / 20.0)
    ...
    def _spectral_denoise(self, signal_data: np.ndarray, force: np.ndarray) -> np.ndarray:
        ...
        reduction_strength = self.synthpostprocess.spectral_reduction_strength * (1.0 - 0.8 * force_env)
        ...
        for f_idx in force_onset_idx:
            ...
            if len(nearby) > 0:
                ...
                if abs(shift) < window // 2:
                    ...
                    crossfade_len = min(self.synthpostprocess.crossfade_samples, 
    ...
    def _dynamic_amplify(self, signal_data: np.ndarray, force: np.ndarray, track_name: str) -> np.ndarray:
        ...
        target_rms = track_rms_targets.get(track_name, self.synthpostprocess.target_rms)
        ...
        # Limit gain
        max_gain_linear = 10 ** (self.synthpostprocess.max_gain_db / 20.0)
        ...
        # Apply compression
        compression = self.synthpostprocess.dynamic_range_compression
    ...
    def _force_weighted_blend(self, dry: np.ndarray, wet: np.ndarray, force: np.ndarray) -> np.ndarray:
        ...
        blend_factor = self.synthpostprocess.dry_wet_mix * (0.5 + 0.5 * force_env)
    ...
    def _blend_tracks(self, tracks: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        for track_name, level in track_levels.items():
            if track_name in tracks and len(tracks[track_name]) > 0:
                ...
                crossfade = min(self.synthpostprocess.crossfade_samples, len(main))
